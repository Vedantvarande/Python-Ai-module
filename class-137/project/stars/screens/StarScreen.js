    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#CFDEE7' }}>
        <Card containerStyle={{ marginTop: 50 }}>
          <Card.Title style={styles.header}>{this.state.star_name}</Card.Title>
          <Card.Image source={image} resizeMode="contain" />
          <View>
            <Text
              style={
                styles.darkDesc
              }>{`Distance from Earth : ${this.state.distance} light years`}</Text>
            <Text
              style={styles.lightDesc}>{`Mass : ${this.state.mass} Suns`}</Text>
            <Text
              style={
                styles.darkDesc
              }>{`Radius : ${this.state.radius} Suns`}</Text>
            <Text
              style={
                styles.lightDesc
              }>{`Surface Gravity : ${this.state.surface_gravity} m/s²`}</Text>
          </View>
        </Card>
      </View>
    );
